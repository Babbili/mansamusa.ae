...
const plugin = (_) => {
    ...
    addFilter('DID_LOAD_ITEM', (item, { query, dispatch }) => new Promise((resolve, reject) => {
        ...
        // get file reference
        const { file } = item;
        if (!query('GET_ALLOW_IMAGE_EDITOR') || !query('GET_IMAGE_EDITOR_INSTANT_EDIT'))
            return resolve(item);
    ...
    const couldTransformFile = (query) => {
        ...
        return (query('GET_IMAGE_EDITOR_WRITE_IMAGE') && imageProcessor && imageReader && imageWriter);
    };
    ...
    addFilter('CREATE_VIEW', (viewAPI) => {
        ...
        const didLoadItem = ({ root, props }) => {
            if (!query('GET_IMAGE_EDITOR_ALLOW_EDIT'))
                return;
            const { id } = props;
            // try to access item
            const item = query('GET_ITEM', id);
    ...
    //#region write image
    addFilter('SHOULD_PREPARE_OUTPUT', (shouldPrepareOutput, { query, item, action }) => new Promise((resolve) => {
        // if is no action we apply initial state
        // if is an action but isn't an image state update skip
        if (action && !/imageState/.test(action.change.key))
